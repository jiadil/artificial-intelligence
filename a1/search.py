...
def astar_search(problem, h_type, h=None, display=False):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    
    # 0 for misplaced, 1 for Manhattan Distance, 2 for max of them, 3 for cube class
    h = ()
    if h_type == 0:
        h = memoize(problem.mp_h, 'h')
    elif h_type == 1:
        h = memoize(problem.md_h, 'h')
    elif h_type == 2:
        h = memoize(problem.mx_h, 'h')
    elif h_type == 3:
...
class StagePuzzle(Problem):
    ...
    def md_h(self, node):
        ...
        for i in range(1, 10):
            sum += abs(self.positions[node.state.index(i)][0] - self.positions[self.goal.index(i)][0]) + abs(self.positions[node.state.index(i)][1] - self.positions[self.goal.index(i)][1])
    
        return sum
    
    # Combined (max of mp and md)
    def mx_h(self, node):
        return max(self.md_h(node), self.mp_h(node))    
